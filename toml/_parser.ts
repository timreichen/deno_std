...
export class Scanner {
  ...
  nextUntilChar(
    ...
    if (!this.isCurrentCharEOL() && /\s/.test(this.char())) {
      ...
      throw new ParseError(`Contains invalid whitespaces: \`${escaped}\``);
    }
  }
  ...
  return function Join(scanner: Scanner): ParseResult<T[]> {
    ...
    while (!scanner.eof()) {
      ...
      if (result.ok) {
        ...
        throw new ParseError(`Invalid token after "${separator}"`);
      }
    }
    return success(out);
  };
...
function kv<T>(
  ...
  return function Kv(
    ...
    if (!sep.ok) {
      throw new ParseError(`key/value pair doesn't have "${separator}"`);
    }
...
function surround<T>(
  ...
  return function Surround(scanner: Scanner) {
    ...
    const result = parser(scanner);
    if (!result.ok) {
      throw new ParseError(`Invalid token after "${left}"`);
    }
    if (!Right(scanner).ok) {
      throw new ParseError(
...
export function BasicString(scanner: Scanner): ParseResult<string> {
  ...
  while (scanner.char() !== '"' && !scanner.eof()) {
    if (scanner.char() === "\n") {
      throw new ParseError("Single-line string cannot contain EOL");
    }
...
export function DateTime(scanner: Scanner): ParseResult<Date> {
  ...
  if (isNaN(date.getTime())) {
    throw new ParseError(`Invalid date string "${dateStr}"`);
  }
...
export function ArrayValue(scanner: Scanner): ParseResult<unknown[]> {
  ...
  if (scanner.char() === "]") {
    ...
    throw new ParseError("Array is not closed");
  }
